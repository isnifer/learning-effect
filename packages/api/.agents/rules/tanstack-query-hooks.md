...
`ticketQueries.ts`, to match the project's singular oRPC namespace convention.
...
Screens and components must import ready domain hooks, such as `useTicketsQuery`, `useCreateTicket`, and
`useUpdateTicketStatus`. They must not assemble these domain queries and mutations inline.
