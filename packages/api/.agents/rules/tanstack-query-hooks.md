...
`taskQueries.ts`, to match the project's singular oRPC namespace convention.
...
Screens and components must import ready domain hooks, such as `useTasksQuery`, `useCreateTask`, and
`useUpdateTaskStatus`. They must not assemble these domain queries and mutations inline.
