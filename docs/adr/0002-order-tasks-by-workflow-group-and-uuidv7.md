...
# Order tasks by workflow group and UUIDv7

The task list represents workflow priority before creation recency. Active work must remain visible
at the top, pending work follows it, and completed work stays at the end of the list.

## Decision

Assign every task status to an ordered workflow group:
...
Sort tasks first by the workflow group and then by `id DESC` within that group.

Task identifiers are UUIDv7 values. Their canonical string representation starts with the embedded
...
The status-to-group mapping must be exhaustive against the domain `TaskStatus` type. Adding a new
...
- Changing a task to an active status moves it into the top group.
- Changing a task to a completed status moves it into the final group.
- A status change does not make a task newer. Its position within the destination group still follows
  its UUIDv7 creation time.
- A newly created `TODO` appears before older pending tasks but after every active task.
- A newly completed task appears before older completed tasks but after every active and pending task.
